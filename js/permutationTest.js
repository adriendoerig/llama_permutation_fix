...
const nodeTreatmentWidth = (d) => {
  if (d.nodeGroup === "resp" || d.nodeGroup === "dsn") return width / 2;
  else if (d.index % 2 == 0) return width > mobileWidth ? trtCenter : trtCenter / 2;
  else return width > mobileWidth ? cntrlCenter : width / 1.35;
};

const nodeTreatmentHeight = (d) => {
  if (d.nodeGroup == "resp") return height / 1.1;
  else if (d.nodeGroup == "llama") return height / 3.5;
  else return height / 1.1;
};

const nodeGroupInitialForceCollide = (d) => {
  if ((d.nodeGroup === "llama") & (width > mobileWidth)) return 35;
  else if (d.nodeGroup === "llama") return 18;
  else return 10;
};

const nodeGroupMoveForceCollide = (d) => {
  if ((d.nodeGroup === "llama") & (width > mobileWidth)) return 29;
  else if (d.nodeGroup === "llama") return 14;
  else return 0;
};

const nodeGroupMoveForceCollideUp = (d) => {
  if (d.nodeGroup == "resp") return 15;
  else if (d.nodeGroup == "llama") return 37;
  else return 15;
};

const nodeInitialXPlacement = (d) => {
  if (d.nodeGroup === "llama") return width > mobileWidth ? width / 2 : width / 4;
  else return width / 5;
};

const nodeInitialYPlacement = (d) => {
  if (d.nodeGroup === "llama") return width > mobileWidth ? height / 3 : height;
  else return height / 1.1;
};
...
// 🦙 FIXED: add colored circles and llama images per group
const llamaGroups = dots.filter((d) => d.nodeGroup === "llama");

// colored background (treatment = red, control = blue)
llamaGroups
  .append("circle")
  .attr("r", 22)
  .attr("cx", 15)
  .attr("cy", 15)
  .attr("fill", (d, i) =>
    i % 2 === 0 ? "rgba(248,131,121,0.6)" : "rgba(131,238,248,0.6)"
  )
  .attr("stroke", "none");

// llama image centered on circle
llamaGroups
  .append("image")
  .attr("xlink:href", "assets/llama1.png")
  .attr("width", 30)
  .attr("height", 30)
  .attr("x", 0)
  .attr("y", 0)
